...
public class ItemDetails_activity extends AppCompatActivity {
    ...
    TextView item_name,item_des,item_price,discounttext,afterdiscount;
    Product_class pc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edit_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (dis.matches("")) {
                    Toast.makeText(ItemDetails_activity.this, "You did not enter any amount", Toast.LENGTH_SHORT).show();

                }
                else
                {
                    updateDiscount(deleteRef,Integer.valueOf(dis));
                    Toast.makeText(ItemDetails_activity.this, "Discount Added", Toast.LENGTH_SHORT).show();
                }
    ...
    private void initialize() {
        ...
        afterdiscount = findViewById(R.id.text_view_afterdiscount);
    }
    void inten_init() {
        Intent in=getIntent();
        ...
        try{
            ...
            discounttext.setText("Current Discount:" + pc.discount.toString()+" %");
            Integer x= pc.price - pc.price * pc.discount / 100;
            afterdiscount.setText("Price after discount:" + x.toString());
        }catch (Exception e){
            Log.e("DEBUGME",e.getMessage());
        }
    }
    void process_intent_data() {
        if(pc.size.get(0))s.setChecked(false);
        else {s.setVisibility(View.GONE);}
        if(pc.size.get(1))m.setChecked(false);
        else {m.setVisibility(View.GONE);}
        if(pc.size.get(2))l.setChecked(false);
        else {l.setVisibility(View.GONE);}
        if(pc.size.get(3))xl.setChecked(false);
        else {xl.setVisibility(View.GONE);}
        if(pc.size.get(4))xxl.setChecked(false);
        else {xxl.setVisibility(View.GONE);}
        if(pc.size.get(5))xl_3.setChecked(false);
        else {xl_3.setVisibility(View.GONE);}
        if(pc.size.get(5) )xl_4.setChecked(false);
        else {xl_4.setVisibility(View.GONE);}
    }
    void Firebase_init() {
